# Übersicht aller Dokumentationen

- diese Übersicht listet alle Dokumentationen auf die für dieses Projekt relevant sind

## 📱 Nutzer Dokumentationen

- Diese docs sind für 'Zuschauer'
- Sie sind für eine Transparente Entwicklung hier verlinkt

| Dokument      | Zweck/ Inhalt                     | Link                           |
|---------------|-----------------------------------|--------------------------------|
| ReadMe        | Hauptseite des Projektes          | [ReadMe](../ReadMe.md)         |
| TODO          | Auflistung der Pläne              | [TODO](docs/TODO.md)           |
| Devlog        | Tagebuch des Projektes            | [Devlog](docs/Devlog.md)       |
| Roadmap       | Ungefährer Projektablauf          | [Roadmap](docs/Roadmap.md)     |
| Assets        | Auflistung aller Assets           | [Assets](docs/Assets.md)       |
| Dungeon       | Dungeon Generierungs Algorithmen  | [Dungeon](docs/Dungeon.md)     |
|               |                                   |                                |

## 👨‍💻 Entwickler Dokumentationen

- Diese docs sind für mich als Entwickler
- Sie sind auch für eine Transparente Entwicklung hier verlinkt

| Development Docs| Zweck/ Inhalt                       | Link                                    |
|-----------------|-------------------------------------|-----------------------------------------|
| Style Guide     | Richtlinien zum verfassen der Docs  | [Style Guide](docs/StyleGuide.md)       |
| Kollisionen     | Auflistung der Kollisionen          | [Kollisionen](docs/Kollisionen.md)      |
| Debug & Archiv  | Auflistung aller Bugs & Fehler      | [Debug](docs/Debugging/DebugArchiv.md)  |
| Setup           | Auflistung aller Dev tools          | [Setup](docs/Setup.md)                  |
|                 |                                     |                                         |

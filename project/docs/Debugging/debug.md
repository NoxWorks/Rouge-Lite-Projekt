# ⚠️ Hier werden Warnungen, Error & Bugs Dokumentiert die während der Entwicklung auftreten

- Es werden nur Script error eingefügt wenn diese "Kompliziert" zu debuggen sind. Also keine Syntax fehler

----

## 📄 Wichtige Links zu anderen Dokumentationen

- Hier zur [TODO](TODO.md)
- Hier zur [Roadmap](Roadmap.md)
- Hier zu den [Kollisionen](kollisionen.md)
- Hier zum [Devlog](devlog.md)
- Hier zurück zum [ReadMe](../../../README.md)

## 📃 Übersicht aller Probleme

- Hier zu einer Übersicht aller [Status-Icons](#status-icon-übersicht)

| Datum        | Kategorie        | Art      | Problem                                          | Status   | Link                                                                           |
|--------------|------------------|----------|--------------------------------------------------|----------|--------------------------------------------------------------------------------|
| 26.01.26     | Ressourcen       | WARNING  | Invalide UID für Sprites                         |   ✅     | [Invalide UIDs bei Sprites](Archiv/invalide_uid_für_sprites.md)                |
| 26.01.26     | Editor           | ERROR    | Externer Editor konnte nicht gefunden werden     |   ✅     | [Externer Editor nicht gefunden](Archiv/externer_editor_nicht_gefunden.md)     |
|              |                  |          |                                                  |          |                                                                                |

----

## Status Icon Übersicht

| Status | Beschreibung         |
|--------|----------------------|
| ✅     | Problem gelöst       |
| ⏳     | Wird bearbeitet      |
| 🔧     | In Arbeit            |
| 🚫     | Ignoriert            |
